...
start "Sentinel Client 1" "%GODOT_EXE%" --path "%PROJECT_DIR%" --position 1350,50 --screen 0 -- --client --host=%HOST% --port=%PORT% --auto-connect --username=Alice
timeout /t 1 /nobreak >nul

REM Start client 2 (bottom-left quadrant) - auto-connect
echo Starting CLIENT 2 (bottom-left)...
start "Sentinel Client 2" "%GODOT_EXE%" --path "%PROJECT_DIR%" --position 50,750 --screen 0 -- --client --host=%HOST% --port=%PORT% --auto-connect --username=Bob
timeout /t 1 /nobreak >nul

REM Start client 3 (bottom-right quadrant) - auto-connect
echo Starting CLIENT 3 (bottom-right)...
start "Sentinel Client 3" "%GODOT_EXE%" --path "%PROJECT_DIR%" --position 1350,750 --screen 0 -- --client --host=%HOST% --port=%PORT% --auto-connect --username=Charlie
